...
PUT_ACTIVITY_URL = api.API_URL + "/activities/{id}"
...
def can_process_activity(activity: Activity) -> bool:
    ...
    elif activity.title not in DEFAULT_TITLES:
        logging.info(
            f"Activity {activity.id} will not be processed because "
...
def update_activity(
    activity_id: int, title: str, description: str, access_token: str
) -> None:
    """
    Updates an activity with a given ID by setting its title and description
    to the ones generated by this program.
    """
    url = PUT_ACTIVITY_URL.format(id=activity_id)
    data = {
        "access_token": access_token,
        "name": title,
        "description": description
    }
    api.put(url, data, api.is_status_200)
    logging.info(
        f"Successfully updated the title/description of activity "
        f"{activity_id}. Title: {title}")
